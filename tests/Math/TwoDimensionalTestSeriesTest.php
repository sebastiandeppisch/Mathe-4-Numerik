...
class TwoDimensionalTestSeriesTest extends Testcase{
	public function testArithmeticalAverage(){
		$s = new TwoDimensionalTestSeries([1, 2, 10, 100], [10, 93, 13, 15]);
		$this->assertEquals(new RationalNumber(113, 4), $s->getArithmeticalAverageX());
		$this->assertEquals(new RationalNumber(131, 4), $s->getArithmeticalAverageY());
	}
	...
	public function testEmpirischeKovarianz(){
		...
		$this->assertEquals(0.844, $s->getEmpirischeKovarianz()->evaluate(), '', 0.001);
	}
	...
	public function testEmpirischerKorrelationskoeffizient(){
		...
		$this->assertEquals(0.816, $s->getEmpirischerKorrelationskoeffizient()->evaluate(), '', 0.001);
	}

	public function testRegressionsGerade(){
		$s = new TwoDimensionalTestSeries([20, 16, 15, 16, 13, 10], [0, 3, 7, 4, 6, 10]);
		$this->assertEquals(-0.98, $s->getRegressionsGeradeA()->evaluate(), '', 0.01);
		$this->assertEquals(19.73, $s->getRegressionsGeradeB()->evaluate(), '', 0.01);
	}
